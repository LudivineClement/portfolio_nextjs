...
const ProjectCard = ({ img, title, github, url, onToggleProjectInfo }) => {
    ...
    <div className="bg-white max-w-[350px] group relative overflow-hidden h-[400px] shadow-sm will-change-transform">
      ...
      <div className='absolute w-full bg-pink-global h-0 z-10 -bottom-5 p-4 group-hover:h-[100px] group-hover:opacity-80 group-hover:bottom-0 transition-bottom duration-500 ease-in-out opacity-90'>
        ...
        <div className='flex justify-center m-3 items-center'>
          <div title='Voir plus' onClick={onToggleProjectInfo}><FontAwesomeIcon icon={faCirclePlus} className='text-black mx-[5px] transition-transform duration-200 opacity-100 ease-in hover:scale-125 cursor-pointer' /></div>
          <a href={github} title='Lien Github' target="_blank" className='text-black mx-[5px] mb-[2px] transition-transform duration-200 opacity-100 ease-in hover:scale-125'>{IconGithub}</a>
          {url ? (
